...
def is_stochastic(
    P: np.ndarray,
    precision: float = ARITHMETIC_PRECISION,
    stochastic_type: str = "right",
) -> bool:
    ...
    if (m == 1 and n > 1) or (m > 1 and n == 1):
        ...
        if stochastic_type == "right":
            ...
            return all(
                [math.fabs(sum(P[i, :]) - 1) < precision for i in range(m)]
            ) and all([math.fabs(sum(P[:, i]) - 1) < precision for i in range(n)])
...
def is_unidirectional_matrix(
    ...
    if stochastic_type == "right":
        ...
        raise RuntimeError(
            r"I haven't implemented that yet chief, for I am but a humble lad"
        )

    return is_stochastic(P, precision, stochastic_type) and is_triangular(
...
def test_stochastic():
    ...
    for test in range(int(tests)):
        ...
        left = np.transpose(
            np.asarray(np.random.dirichlet(np.ones(test_dimension), test_dimension))
        )

        row = np.asarray(np.random.dirichlet(np.ones(test_dimension), 1)).reshape(
            1, test_dimension
        )
        column = np.asarray(
            np.random.dirichlet(np.ones(test_dimension), 1).reshape(test_dimension, 1)
        )
